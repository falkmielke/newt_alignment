...
props = skimeas.regionprops_table(
    np.array(yellow_mask, dtype = int),
    properties=('centroid', 'orientation', 'axis_major_length', 'axis_minor_length'),
)

# we get two centroids: centroid-0 is that of the void, centroid-1 that of the mask.

rotated_newt = skitrafo.rotate(cropped_newt, props["orientation"][0], center = props["centroid-1"])
rotated_yellow = skitrafo.rotate(yellow_mask, props["orientation"][0], center = props["centroid-1"])

standard_newt = crop_mask(rotated_newt, rotated_yellow, return_crop_mask = False, extend_px = int(-props["axis_minor_length"][0]/2))
